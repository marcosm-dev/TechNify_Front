<template>
  <v-container>
    <v-app-bar
      ...
      :class="$vuetify.breakpoint.smAndDown ? 'navbar-xs' : 'navbar'"
    >
      <v-row>
      ...
      <v-row>
        <v-row justify="end">
          <v-col
            :class="$vuetify.breakpoint.smAndUp ? 'navbar-ipad' : 'navbar-filter'"
            v-if="appBarVisible"
            cols="6"
          >
            <div class="hidden-md hidden-sm">
              <v-select v-model="selected" :items="filterTypes" required @change="searchFunction" placeholder="Select Event Type"></v-select>
            </div>
          ...
          <v-icon class="mr-n2" v-if="appBarVisible" @click="searchFunction">mdi-magnify</v-icon>
          <v-menu bottom left>
            <template v-if="userMenu" v-slot:activator="{ on }">
              <v-btn
                style="align-self: center"
                :class="$vuetify.breakpoint.lgAndUp ? '' : 'mr-n2 mt-7 icons'"
                class="ml-10 mr-4"
                dark
                icon
                v-on="on"
            ...
            <v-list>
              <v-list-item-group active-class="deep-blue--text text--accent-4">
                ...
                <v-list-item to="/calendar">
                  <v-list-item-icon>
                    <v-list-item-title class="headline">My Calendar</v-list-item-title>
                  </v-list-item-icon>
                  <v-spacer></v-spacer>
...
export default {
  data () {
    return {
      menu: false,
      filterTypes: ['ALL', 'JAVASCRIPT', 'UX', 'OTHERS', 'WEB DEVELOPER', 'HACKATONS'],
      selected: '',
      dates: [],
      search: true,
